...
{
	Color general = (_mode == ST_ORANGE) ? Yellow : Red_BG;
...
void Display::DrawCarImp(int _y, int _x, Color _color)
{
	SetColor(_color);
...
void Display::DrawCar(MovinEntity _entity, Color _color)
{
	struct winsize size;
	if (ioctl(0, TIOCGWINSZ, (char *) &size) < 0)
    	printf("TIOCGWINSZ error");
...
//
	...
	switch(_entity)
	{
		case NORTH:	DrawCarImp(size.ws_row/2+y/2-3, size.ws_col/2-5, _color);
			break;
		case SOUTH:	DrawCarImp(size.ws_row/2-y/2+3, size.ws_col/2+4, _color);
			break;
		case EAST:	DrawCarImp(size.ws_row/2+2, size.ws_col/2-16, _color);
			break;
		case WEST:	DrawCarImp(size.ws_row/2-2, size.ws_col/2+16, _color);
			break;
