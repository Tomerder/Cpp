...
StructuredBuffer<float>   g_bufPosVelo;
...
VSParticleDrawOut VSParticleDraw(VSParticleIn input)
{
	...
	output.pos.x = g_bufPosVelo[input.id*8 + 0];
	output.pos.y = g_bufPosVelo[input.id*8 + 1];
	output.pos.z = g_bufPosVelo[input.id*8 + 2];

    float mag = g_bufPosVelo[input.id*8+7]/9;
