...
struct particle
{
	particle() restrict(direct3d,cpu) {}
    float_4 pos;
    float_4 vel;
};
...
public:
	nbody(ID3D11Device* pd3dDevice) :
		_accl_view(direct3d::create_accelerator_view(reinterpret_cast<IUnknown *>(pd3dDevice)))
	{
...
private:
	...
	// CPU-GPU functions
	static float_4 to_float_4(D3DXVECTOR4 v) restrict(direct3d,cpu)
	{
		return float_4(v.x, v.y, v.z, v.w);
	}

	//// GPU based functions
	static void bodybody_interaction(float_4 &acc, const float_4 pos, const float_4 A) restrict(direct3d);
