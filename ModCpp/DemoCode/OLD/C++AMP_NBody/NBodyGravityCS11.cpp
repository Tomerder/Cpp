...
#define MAX_GPU_PARTICLES           (57*1024)       // the maximum number of particles in the n-body simulation
#define MAX_CPU_PARTICLES           (15*1024)           // the maximum number of particles in the CPU n-body simulation
...
void InitApp()
{
    ...
    g_HUD.AddStatic( IDC_NBODIES_LABEL, szTemp, -20, iY += 34, 125, 22 );
    g_HUD.AddSlider( IDC_NBODIES_SLIDER, -20, iY += 34, 170, 22, 1, MAX_GPU_PARTICLES/PARTICLES_STEP_SIZE );
    g_HUD.AddComboBox( IDC_COMPUTETYPECOMBO, -20, iY += 34, 190, 26, L'G', false, &pComboBox );
    ...
    if( pComboBox )
    {
		...
		pComboBox->AddItem( L"AMP Flat", ( void* )CT_GPUSINGLESIMPLE );
        pComboBox->AddItem( L"AMP Tiled", ( void* )CT_GPUSINGLETILE );
        pComboBox->AddItem( L"GPU Multi Device", ( void* )CT_GPUMULTI );
    }
    
    g_HUD.GetSlider( IDC_NBODIES_SLIDER )->SetValue( (DEFAULT_CPU_PARTICLES / PARTICLES_STEP_SIZE) );
...
HRESULT CreateParticlePosVeloBuffers( ID3D11Device* pd3dDevice )
{
    ...
    for(size_t i=0; i<MAX_GPU_PARTICLES; i += PARTICLES_STEP_SIZE)
    {
        LoadParticles( &g_pParticles[i], 
            D3DXVECTOR3(g_fSpread,0,0), D3DXVECTOR4(0,0,-8,1/10000.0f / 10000.0f),
            g_fSpread, PARTICLES_STEP_SIZE / 3 );
        LoadParticles( &g_pParticles[i + (PARTICLES_STEP_SIZE / 3)],
            D3DXVECTOR3(-g_fSpread,0,0), D3DXVECTOR4(0,0,8,1/10000.0f / 10000.0f),
            g_fSpread, PARTICLES_STEP_SIZE / 3 );
        LoadParticles( &g_pParticles[i + 2*(PARTICLES_STEP_SIZE / 3)],
            D3DXVECTOR3(0,0,g_fSpread*15.0f), D3DXVECTOR4(0,0,-60,1/10000.0f / 10000.0f),
            g_fSpread, (PARTICLES_STEP_SIZE + 2) / 3 );
    }
#endif
    ...
    DescRV.Buffer.NumElements = (MAX_GPU_PARTICLES * sizeof(particle)) / sizeof(float);
    ...
    case CT_CPUSINGLE:
        {
            ...
            Swap(g_pParticles, g_pParticlesTemp);
        }
        break;
	case CT_CPUMULTI:
		{
    ...
    case IDC_COMPUTETYPECOMBO:
        {
            ...
            if(g_nNumParticles > MAX_CPU_PARTICLES && (g_eComputeType == CT_CPUSINGLE || g_eComputeType == CT_CPUMULTI))
            {
                ...
                g_HUD.GetSlider( IDC_NBODIES_SLIDER )->SetValue( MAX_CPU_PARTICLES / PARTICLES_STEP_SIZE );
            }
    ...
    case IDC_NBODIES_SLIDER:
        {
            ...
            g_nNumParticles = pSlider->GetValue() * PARTICLES_STEP_SIZE;
            if(g_nNumParticles > MAX_CPU_PARTICLES && (g_eComputeType == CT_CPUSINGLE || g_eComputeType == CT_CPUMULTI))
            {
                // cap the number of particles in the CPU version
                g_nNumParticles = MAX_CPU_PARTICLES;
