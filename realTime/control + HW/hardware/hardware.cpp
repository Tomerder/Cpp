...
void Hardware::InterruptBind(InterruptHandlerFunc _func, EmulationEvent _line)
{
	cout << "Hardware::InterruptBind  " << (unsigned)_func << "    "  << _line << endl; 
...
//execute int.handler
void Hardware::InterruptEmulate(EmulationEvent _line)
{
	cout << "Hardware::InterruptEmulate" << endl;

	cout << m_areInterruptsEnabled << "  hundler adr:  "  << (unsigned)(m_interruptVector[_line]) << endl ;

	if (m_areInterruptsEnabled && m_interruptVector[_line] ) // && m_InterruptLinesStatus[_line])
	{
		cout << "if (m_areInterruptsEnabled && m_InterruptLinesStatus[_line])" << endl;
		(m_interruptVector[_line])();
	}	
}

void Hardware::InterruptLineDisable(EmulationEvent _line)
{
	m_InterruptLinesStatus[_line] = false;
}

void Hardware::InterruptLineEnable(EmulationEvent _line)
{
	m_InterruptLinesStatus[_line] = true;
}
...
void Hardware::Bind_1MicroSecondTick(EmulationEvent _line)
{
	m_1MicroSecondTick[m_microTickCounter++] = _line;
}

void Hardware::Bind_1MilliSecondTick(EmulationEvent _line)
{
	m_1MilliSecondTick[m_milliTickCounter++] = _line;
}

void Hardware::Bind_1SecondTick(EmulationEvent _line)
{
